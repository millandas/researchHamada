...
Analysing risks of climate transition requires quality automated systems that are capable of projecting energy generation predictions under various scenarios whilst also quantifying uncertainty factors. Nuclear energy is a low-carbon electricity source alternative to the status quo systems that plays a critical role in the decarbonisation outline constructed by the International Energy Agency (IEA) Net Zero 2050 scenario. 

This research project develops an end-to-end pipeline that covers multiple data science tasks. The pipeline was developed to engineer features for capacity factor estimation and generation calculation, as well as projecting region and/or global nuclear generation to the year 2050 under multiple scenarios. Moreover, it can compare these projections against the IEA Net Zero targets to identify gaps and calculate the avoided CO$_2$ emissions from the replacement of fossil fuel energy generation with nuclear.
This deploys via an interactive streamlit dashboard.
...
The pipeline is made up of a modular architecture comprised of 5 processing stages, each of which is implemented independently as a Python class.
...
\end{tikzpicture}
\caption{System Architecture: End-to-end pipeline from data ingestion to deployment}
...
Table~\ref{tab:tech-stack} summarises the technologies and frameworks used in the pipeline.
...
The primary data source is the \textbf{Global Nuclear Power Tracker} (September 2025 release), maintained by Global Energy Monitor. This dataset provides comprehensive information on nuclear power plants around the world (see table below).
...
\begin{table}[H]
\centering
\caption{Key Data Fields in Global Nuclear Power Tracker}
\label{tab:data-fields}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Category} & \textbf{Attributes} \\
\midrule
Identification & Project Name, Unit Name, Country/Area, Region, Subregion \\
Technical Specifications & Reactor Type, Model, Capacity (MW) \\
Timeline & Start Year, Commercial Operation Date, Retirement Year \\
Operational Status & Operating, Announced, Construction, Cancelled, Shelved, Retired \\
Geospatial & Latitude, Longitude, State/Province \\
...
Table~\ref{tab:status-dist} shows the distribution of plants by operational status:
...
The IEA Net Zero by 2050 scenario (NZE2021\_AnnexA.csv) provides a proposed set of nuclear generation targets that would be required in order to achieve net-zero emissions on a global scale by 2050. 
...
\begin{itemize}[noitemsep]
    \item Regional  (4 global regions)
    \item Subregional 
    \item Country-level  (61 countries)
    \item Status-based
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Capacity Factors} follow a Normal distribution:
    \begin{equation}
    ...
    \item \textbf{Construction Delays} are Scenario-dependent:
    \begin{equation}
...
\begin{itemize}[noitemsep]
    \item P10: Lower bound / pessimistic
    \item P50: Central estimate / median
    \item P90: Upper bound / optimistic
\end{itemize}
...
Features include sequential dependency management, skip-if-exists capability for incremental runs, comprehensive logging (file + console), execution time tracking per stage, error handling with detailed tracebacks, output cleaning for fresh runs.
...
Logs are written to \texttt{logs/pipeline\_YYYYMMDD\_HHMMSS.log} with timestamps for each operation, stage start/completion status, execution duration per stage, error messages with tracebacks, and output file sizes
...
The dashboard contains 7 interactive pages, summarised in Table~\ref{tab:dashboard-pages}.

\begin{table}[H]
\centering
\caption{Dashboard Pages and Features}
\label{tab:dashboard-pages}
\begin{tabular}{@{}lp{9cm}@{}}
\toprule
\textbf{Page} & \textbf{Features} \\
\midrule
Global Overview & Key metrics, status distribution, regional distribution, global timeline \\
Regional Analysis & Regional projections, capacity distribution, generation share \\
Projections 2025--2050 & Multiple visualisation types, growth rate analysis, milestone years \\
IEA Scenario Comparison & Gap analysis, regional gaps, alignment status \\
Avoided Emissions & Annual and cumulative emissions, car equivalent metrics \\
Pipeline Analysis & Operating vs pipeline capacity, contribution breakdown \\
Statistical Analysis & Distribution plots, correlations, regression visualisation \\
\bottomrule
\end{tabular}
\end{table}
...
The interactive dashboard includes the features summarised in Table~\ref{tab:interactive-features}.

\begin{table}[H]
\centering
\caption{Interactive Dashboard Features}
\label{tab:interactive-features}
\begin{tabular}{@{}p{3.5cm}p{9cm}@{}}
\toprule
\textbf{Feature} & \textbf{Description} \\
\midrule
Sidebar Filters & Year range slider, region multiselect, scenario dropdown for dynamic data filtering \\
Plotly Charts & Interactive hover tooltips, zoom, and pan capabilities for detailed exploration \\
Responsive Design & Optimised layouts for desktop, tablet, and mobile viewing \\
Data Caching & Performance optimisation using \texttt{@st.cache\_data} decorators \\
\bottomrule
\end{tabular}
\end{table}
...
The projections show nuclear generation to grow up to between 5000 to 7000 TWh by 2050, depending on the scenario.
...
The table shows that the varying scenarios have a difference of 2,200 TWh for their predictions by 2050 and therefore reflecting that there exists a substantial uncertainty in the deployment rates of future nuclear energy systems. The aggressive scenario’s output is dependent on accelerated construction timelines and better policy support whereas the conservative scenario expects further delays or projects being cancelled.

\subsection{IEA Gap Analysis}
The gap analysis suggests that nuclear generation will not reach IEA targets by 2030 outside of the aggressive scenario.
...
The 15.3\% miss that is projected for the year 2030 tells us that the status quo of construction pipelines are insufficient if we want to meet our near-term IEA goals. The gap, however, does actually narrow over time and actually reverses by 2050 (under the base scenario).

\subsection{Avoided Emissions}
The predicted avoided cumulative CO$_2$ emissions over the next 25 years from nuclear generation replacing fossil fuels reach up to 50 GtCO$_2$.
...
The cumulative 30 to 50 GtCO$_2$ that could be avoided represents an approximate 5 to 8\% of the remaining global  carbon budget for 1.5°C. It effectively proves the potential nuclear energy can have in its contribution to climate change mitigation efforts.
...
The complete codebase is version-controlled on GitHub. Features are traceable via commit history, and branches are used for feature development and branch isolation. Documentation for the codebase exists inside the README as well as in inline comments.
...
pandas, numpy, openpyxl, matplotlib, seaborn, plotly, scipy, statsmodels, scikit-learn, streamlit, jupyter, papermill
\end{lstlisting}
...
A complete data pipeline with an end-to-end automation workflow that outputs to an interactive dashboard. 
A Monte Carlo simulation to quantify uncertainties.
Trivariate scenario analyses (base, conservative, aggressive) to support decisions.
Gap analysis against NetZero 2050 to assess alignment with IEA targets.
Quantified avoided CO_2 emissions with added per-region support.
Modular codebase enables reproducible and scalable results.


\subsection{Value for Climate Analytics}

The structure of the pipeline enables reproducibility through the processing of the data. New data can be used to produce updated predictions and larger datasets or larger-scale datasets can be used to scale the analytics.
